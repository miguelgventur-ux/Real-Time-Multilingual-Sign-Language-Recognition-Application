...
=====================================
Evaluates a trained model on a held-out test dataset.

The test data directory (CollectedDataTesting) must follow the same structure
as CollectedData:
...
MODEL_DIR    = r"C:\Diss\Models\Dual_60_Model_Mutual"   # folder produced by training script
TEST_DIR     = r"C:\Diss\CollectedDataTesting"           # held-out test recordings
OUTPUT_DIR   = r"C:\Diss\Models\Dual_60_Model_Mutual\TestResults"

# Which model file to evaluate — set to 'selected_model.h5' to use the best
# snapshot chosen by the checkpoint audit, or 'best_model.h5' for the
# ModelCheckpoint result. You can also pass a snapshot path directly.
...
label_map   = config['label_map']          # {"ASL_baby": 0, "BSL_baby": 1, ...}
...
with open(summary_csv, 'w', newline='') as f:
    ...
    writer.writerow([])   # separator before per-language breakdown
    ...
    writer.writerow([])   # separator before model info
...
annot_array = np.where(cm_norm > 0,
                       np.vectorize(lambda v: f"{v:.2f}")(cm_norm),
                       "")
...
if len(LANGUAGES) > 1:
    ...
    # present_names is sorted by class index, which follows alphabetical label
...
for lang in LANGUAGES:
    ...
    # --- figure sizing: scale by class count but enforce sensible min/max ---
    cell_size = 0.55          # inches per cell
    fig_h     = max(4, n_lang * cell_size + 2.5)    # rows + headroom
    fig_w     = max(5, n_cols * cell_size + 3.0)    # cols + label space
    fig, ax   = plt.subplots(figsize=(fig_w, fig_h))

    # --- font sizes: cap so they never blow up on a 1-class matrix ---
