...
SIGNS = [
    ...
    'color', 'computer', 'corn', 'drink', 'eat',
    # everyday actions
    'sleep', 'walk', 'run', 'sit', 'stand',
    'help', 'stop', 'go', 'come', 'wait',
    'finish', 'want', 'need', 'like', 'love',
    'hate', 'know', 'think',
    # objects and places
    'water', 'food', 'house', 'school', 'car',
    'phone', 'money', 'door', 'table',
    # people
    'me', 'you', 'friend', 'mother', 'father',
    'baby', 'teacher', 'doctor',
    # descriptors
    'hot', 'cold', 'good', 'bad', 'big',
    'small', 'fast', 'slow', 'happy', 'sad',
]

LANGUAGE = "ASL"
OUTPUT_DIR = r"C:\Diss\CollectedData"
RECORDINGS_PER_SIGN = 30
SEQUENCE_LEN = 45
...
def extract_two_hand_landmarks(frame_rgb):
    """ Returns (126) array, sorted by wrist x so ordering is consistent every frame.
    Missing hand slots filled with zeros. """
...
def draw_top_bar(frame, sign, sign_idx, total_signs, saved_count, state, hand_count):
    ...
    cv2.putText(frame, f"Saved: {saved_count}/{RECORDINGS_PER_SIGN}", (w - 240, 35), cv2.FONT_HERSHEY_DUPLEX, 0.75, GREEN, 2, cv2.LINE_AA)
...
def draw_recording_dot(frame):
    if int(time.time() * 2) % 2 == 0: cv2.circle(frame, (frame.shape[1] - 30, 148), 10, RED, -1)
...
def main():
    ...
    while sign_idx < len(SIGNS):
        ...
        draw_top_bar(frame, sign, sign_idx, len(SIGNS), saved_count, state, hand_count)
