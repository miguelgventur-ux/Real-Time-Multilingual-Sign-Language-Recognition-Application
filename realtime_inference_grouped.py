"""
Real-Time Sign Language Recognition — Webcam Inference
...
# ── CONFIG ────────────────────────────────────────────────────────────────────
MODEL_DIR      = r"C:\Diss\Model_grouped_10"
CONF_THRESHOLD = 0.4
SMOOTH_WINDOW  = 5
SETTLE_DELAY   = 0.0    # seconds before showing prediction after hand appears
...
def predict(frame_buffer):
    ...
    idx   = np.argmax(probs)
    return id_to_sign[idx], float(probs[idx])


# ── DRAWING ───────────────────────────────────────────────────────────────────
def draw_top_bar(frame, hand_count, current_sign, current_conf, show_prediction):
    h, w = frame.shape[:2]
    ...
    if show_prediction:
        ...
        cv2.putText(frame, "---", (15, 68),
                    cv2.FONT_HERSHEY_DUPLEX, 1.8, (60, 60, 60), 2, cv2.LINE_AA)

    # Confidence top right
    if show_prediction:
        conf_text = f"{current_conf:.0%} confidence"
...
def draw_bottom_bar(frame, buffer_fill):
    ...
    # Controls
    cv2.rectangle(frame, (0, h - 28), (w, h), DARK, -1)
...
def draw_recording_dot(frame):
    """Flashing dot to show system is actively processing."""
...
while True:
    ...
    # ── DRAW UI ───────────────────────────────────────────────────────────────
    draw_top_bar(frame, len(hand_landmarks_list), current_sign, current_conf, hand_settled)
